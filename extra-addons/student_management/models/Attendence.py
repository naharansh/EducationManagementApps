...
class Attendence(models.Model):
    ...
    total_present_students = fields.Integer(string="Total Present Students", compute="_compute_totals", store=True)
    total_present_days = fields.Integer(string="Total Present Days", compute="_compute_totals", store=True)

    @api.depends('line_ids.present_days', 'line_ids.absent_days')
    def _compute_totals(self):
        for rec in self:
            # count students who have at least 1 present day
            rec.total_present_students = sum(1 for line in rec.line_ids if line.present_days > 0)
            # sum of all present days
            rec.total_present_days = sum(line.present_days for line in rec.line_ids)
    ...
    def action_open_graph(self):
        return {
            ...
            'res_model': 'edu.monthly.attendence.line',
            'view_mode': 'graph',
            'views': [(self.env.ref('student_management.view_attendance_graph').id, 'graph')],
...
class MonthlyAttendenceLine(models.Model):
    ...
    class_capacity = fields.Integer(string="Class Capacity", related='report_id.class_id.capacity', store=True, readonly=True)
    month = fields.Selection(
    selection=[
        ('01','January'), ('02','February'), ('03','March'),
        ('04','April'), ('05','May'), ('06','June'),
