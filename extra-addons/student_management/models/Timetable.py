...
class ClassTimetable(models.Model):
    ...
    # Auto-sync timetable sessions into daily classes
    def action_sync_daily_classes(self):
        Daily = self.env["edu.class.daily"]

        for rec in self:
            # Delete old daily classes
            # rec.daily_class_ids.unlink()

            # Recreate daily classes from timetable sessions
            daily_vals = []
...
class TimetableSession(models.Model):
    ...
    def create(self, vals):
        ...
        rec.line_id.timetable_id.action_sync_daily_classes()
    ...
    def _onchange_session_fields(self):
        if self.line_id:
            self.line_id.timetable_id.action_sync_daily_classes()